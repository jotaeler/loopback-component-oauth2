...
var url = require('url')
  ...
  , ClientJWTBearerStrategy = require('./strategy/jwt-bearer').Strategy,
    LoginCertStrategy = require('passport-client-cert').Strategy,
...
module.exports = function(app, options) {
  ...
  function userLoginCert(req, certificate, done) {
    debug('userLoginCert: %s', certificate);
    models.users.findByDni(certificate.serialNumber, function(err, user){
      if (err) {
        return done(err);
      }
      if (!user) {
  ...
  if (options.loginPath !== false) {
    ...
    passport.use('login-certificate', new LoginCertStrategy({ passReqToCallback: true },userLoginCert));
    ...
    app.post(options.loginPath || '/login',
      passport.authenticate('login-certificate',
      { successReturnToOrRedirect: '/web',
    ...
    //Activar rejectUnauthorized para que no acepte certificados firmados por otra CA
    app.middleware('initial:after', '/loginCert', function(req,res,next){
      var opt = {
        ...
        //SSLv3 tiene un fallo de seguridad llamado POODLE por lo que deshabilito SSLv3
      ...
      var cert=req.socket.renegotiate(opt,function(err){
        if (err) {
          console.log(err);
        }
